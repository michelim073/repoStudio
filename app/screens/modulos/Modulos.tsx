...
const Modulos = () => {
  ...
  useEffect(() => {
    const sub = DataStore.observeQuery(ModulosCursos, (c) =>
      c.cursosID.eq(id), {sort: m => m.index(SortDirection.ASCENDING) }
    ).subscribe(({ items }) => {
      setModulos(items);
  ...
  const crearTituloClase = async () => {
    ...
    try {
      const chatClases = await DataStore.save(new ChatRoomClases({}));
    ...
    <SafeAreaView style={styles.container}>
            ...
            <Pressable onPress={handleExpandPress}>
              {isVisible && (
                <View style={{flexDirection:'row', alignItems:'center'}}>
                  <Text style={{fontWeight:'900',marginRight:2, color:'gray'}}>Modulo</Text>
                  <FontAwesome
                    name="plus-circle"
                    size={25}
                    color={"gray"}
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Accordion
            value={item}
            deleteModuloAlert={deleteModuloAlert}
      ...
      >
        <View style={styles.cajaInputs}>
          ...
          <View style={{ width: 50 }}>
            <Text>Clase</Text>
            <Text style={[styles.TextInput, { alignItems: "center" }]}>
              {indexClase}
            </Text>
          </View>
